...
$bed_numbers_fact = $bed_profile->Stationar()->whereBetween('date_stationar', [$start_date, $end_date])->sum('$bed_numbers_fact')-sum(); 

$avg_bed_occupancy = $beddays_spent/$bed_numbers_fact/$days;
$avg_year_prediction = ($avg_bed_occupancy/$days)*365;
$avg_treat_dur = $beddays_spent/(($received+$died+$discharged)/2);
$bed_rotation = ($discharged+$died)/$bed_numbers_fact/$days;
$hospital_lethality = $died/($discharged+$died);
